...
exports.run = async(client, message, args) => {
    ...
    **${prefix}kayıtlog ayarla #kayıtlog**
    **${prefix}kayıtlog sıfırla** `)
    .setColor("BLUE"));

if(args[0] == "ayarla") {
    if (!channel) return message.channel.send("**Lütfen bir Kanal etiketleyip tekrar deneyin.**")
     db.set(`kayitlog_${message.guild.id}`, channel.id)
...
if(args[0] == "sıfırla") {
    ...
    if(!db.has(`kayitlog_${message.guild.id}`)) return message.channel.send("Kayıt Log Kanalı ayarlanmamış ki sıfırlıyayım")
